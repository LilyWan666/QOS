...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--state",
        required=True,
        help="Path to shared toolchain state.json",
    )
...
def main() -> int:
    ...
    is_allowed, allowed_actions = assert_action_allowed(state, "repro_preflight")
    if not is_allowed:
        payload = blocked_action_payload(
            tool="repro_preflight",
            action="repro_preflight",
            state=state,
            next_allowed_actions=allowed_actions,
        )
        append_history(state, "repro_preflight", payload)
        state["last_step"] = "repro_preflight"
    ...
    attempt_dir = attempts_root / f"attempt_{ordinal:04d}_preflight"
    ...
    if attempt["status"] in {"success", "preflight_only"}:
        set_fsm_state(state, "PREFLIGHT")
    else:
        set_fsm_state(state, "PREFLIGHT_FAILED")
