...
def main() -> int:
    ...
    is_allowed, allowed_actions = assert_action_allowed(state, "run_regression_tests")
    if not is_allowed:
        payload = blocked_action_payload(
            tool="repro_run_regression_tests",
            action="run_regression_tests",
            state=state,
            next_allowed_actions=allowed_actions,
        )
        append_history(state, "repro_run_regression_tests", payload)
        state["last_step"] = "repro_run_regression_tests"
    ...
    commands = plan.get("regression_tests", [])
    if not isinstance(commands, list) or not commands:
        payload = {
            "tool": "repro_run_regression_tests",
            "status": "failed",
            "reason": "missing regression_tests in state.test_plan",
        }
        append_history(state, "repro_run_regression_tests", payload)
        state["last_step"] = "repro_run_regression_tests"
        state["last_status"] = "regression_test_failed"
        set_fsm_state(state, "REGRESSION_TEST")
    ...
    state["last_step"] = "repro_run_regression_tests"
    state["last_status"] = "regression_test_success" if ok else "regression_test_failed"
    state["last_regression_test"] = payload
    set_fsm_state(state, "REGRESSION_TEST")
    payload["fsm_state"] = get_fsm_state(state)
    payload["next_allowed_actions"] = next_actions(state)
    append_history(state, "repro_run_regression_tests", payload)
