...
def evaluate(path):
    ...
    pts = [_get_pair_metrics(i) for i in top_idx]
    step = 0.025
    if pts:
        utils = [u for (u, _f) in pts]
        lo = min(utils)
        hi = max(utils)
        if hi == lo:
            bin_edges = [round(lo, 3), round(lo + step, 3)]
        else:
            num = int(np.ceil((hi - lo) / step)) + 1
            bin_edges = [round(lo + step * i, 3) for i in range(num + 1)]
    else:
        bin_edges = []
    avg_bin_fid = _avg_bin_fidelity(pts, bin_edges)
    score = avg_bin_fid if not np.isnan(avg_bin_fid) else 0.0
    ...
    if config.EVAL_MODE == "both":
        ...
        base_pts = [_get_pair_metrics(i) for i in base_idx]
        base_avg_bin_fid = _avg_bin_fidelity(base_pts, bin_edges)
        print(
            f"[Evaluator] baseline avg-fid: {base_avg_fid:.4f} "
            f"avg-bin-fid: {base_avg_bin_fid:.4f}",
            flush=True,
        )
