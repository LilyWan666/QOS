...
FEATURES=(
  "number_instructions_max"
  "num_nonlocal_gates_max"
  "number_instructions_avg"
  "num_nonlocal_gates_avg"
  "depth_max"
  "number_instructions_diff"
  "num_nonlocal_gates_diff"
  "num_measurements_diff"
)

for feature in "${FEATURES[@]}"; do
  ts="$(date +%Y%m%d_%H%M%S)"
  feature_tag="$(printf '%s' "${feature}" | tr -cs '[:alnum:]_-' '_')"
  job_name="qwen25_p8_${feature_tag}_i200"
